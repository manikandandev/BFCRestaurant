...
<div id="_desktop_cart">
  <div class="blockcart cart-preview {if $cart.products_count > 0}active{else}inactive{/if}" data-refresh-url="{$refresh_url}">
    <div class="header blockcart-header dropdown js-dropdown">
		...
		<a class="top-cart" rel="nofollow" href="{$cart_url}" {if $cart.products_count > 0} data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"{/if}> 
			<i class="material-icons shopping-cart">shopping_cart</i>
			<span class="hidden-sm-down">{l s='shopping cart' d='Shop.Theme.Checkout'}</span>
			<span class="cart-products-count">{$cart.products_count} {l s='item' d='Shop.Theme.Checkout'}</span>
		...
		<div class="cart_block block exclusive dropdown-menu">
			<div class="block_content">
			  ...
			  <div class="checkout card-block">
				  ...
				  {else}
					<a href="{$urls.pages.order}" rel="nofollow">
						<button type="button" class="btn btn-primary">{l s='Checkout' d='Shop.Theme.Actions'}</button>
					</a>					
				  {/if}
				</div>
