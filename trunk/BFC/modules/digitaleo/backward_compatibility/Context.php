...
class Context
{
    ...
    {
        return isset($_SERVER['HTTP_USER_AGENT']) && (bool)Configuration::get('PS_MOBILE_DEVICE') && !Context::getContext()->cookie->no_mobile;
    }
...
class CustomerBackwardModule extends Customer
{
    ...
    public function isLogged($with_guest = false)
    {
        ...
        if ($this->logged == 1 && $this->id && Validate::isUnsignedId($this->id) && Customer::checkPassword($this->id,
                $this->passwd)
        ) {
            return true;
        }
