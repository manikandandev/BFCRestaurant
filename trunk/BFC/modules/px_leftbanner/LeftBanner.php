...
class LeftBanner extends ObjectModel
{
	...
	public static $definition = array(
		'table' => 'pxleftbanner_slides',
		'primary' => 'id_pxleftbanner_slides',
		'multilang' => true,
		'fields' => array(
	...
	public function add($autodate = true, $null_values = false)
	{
		...
		$res &= Db::getInstance()->execute('
			INSERT INTO `'._DB_PREFIX_.'pxleftbanner` (`id_shop`, `id_pxleftbanner_slides`)
	...
	public function delete()
	{
		...
		$res &= Db::getInstance()->execute('
			DELETE FROM `'._DB_PREFIX_.'pxleftbanner`
			WHERE `id_pxleftbanner_slides` = '.(int)$this->id
		);
	...
	public function reOrderPositions()
	{
		...
		$max = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			...
			WHERE hss.`id_pxleftbanner_slides` = hs.`id_pxleftbanner_slides` AND hs.`id_shop` = '.(int)$id_shop
		);
		...
		$rows = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT hss.`position` as position, hss.`id_pxleftbanner_slides` as id_slide
			FROM `'._DB_PREFIX_.'pxleftbanner_slides` hss
			LEFT JOIN `'._DB_PREFIX_.'pxleftbanner` hs ON (hss.`id_pxleftbanner_slides` = hs.`id_pxleftbanner_slides`)
