...
class Px_HomeSlide extends ObjectModel
{
	public $title;
	public $description;
	public $url;
	...
	public static $definition = array(
		'table' => 'pxhomeslider_slides',
		'primary' => 'id_pxhomeslider_slides',
		'multilang' => true,
		'fields' => array(
	...
	public function add($autodate = true, $null_values = false)
	{
		...
		$res &= Db::getInstance()->execute('
			INSERT INTO `'._DB_PREFIX_.'pxhomeslider` (`id_shop`, `id_pxhomeslider_slides`)
	...
	public function delete()
	{
		...
		$res &= Db::getInstance()->execute('
			DELETE FROM `'._DB_PREFIX_.'pxhomeslider`
			WHERE `id_pxhomeslider_slides` = '.(int)$this->id
		);
	...
	public function reOrderPositions()
	{
		...
		$max = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			...
			WHERE hss.`id_pxhomeslider_slides` = hs.`id_pxhomeslider_slides` AND hs.`id_shop` = '.(int)$id_shop
		);
		...
		$rows = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT hss.`position` as position, hss.`id_pxhomeslider_slides` as id_slide
			FROM `'._DB_PREFIX_.'pxhomeslider_slides` hss
			LEFT JOIN `'._DB_PREFIX_.'pxhomeslider` hs ON (hss.`id_pxhomeslider_slides` = hs.`id_pxhomeslider_slides`)
			WHERE hs.`id_shop` = '.(int)$id_shop.' AND hss.`position` > '.(int)$this->position
		);

		foreach ($rows as $row)
		{
			$current_slide = new Px_HomeSlide($row['id_slide']);
