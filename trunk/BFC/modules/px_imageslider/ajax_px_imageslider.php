...
include_once('px_imageslider.php');

$home_slider = new Px_ImageSlider();
$slides = array();
...
if (Tools::getValue('action') == 'updateSlidesPosition' && Tools::getValue('slides'))
{
	...
	foreach ($slides as $position => $id_slide)
		$res = Db::getInstance()->execute('
			UPDATE `'._DB_PREFIX_.'pxhomeslider_slides` SET `position` = '.(int)$position.'
			WHERE `id_pxhomeslider_slides` = '.(int)$id_slide
		);
